...
const Burger = () => {
    ...
    <div className={`burger__menu ${isActive ? "active" : ""}`}>
      <div className={`burger__overlay ${isDarkTheme ? "" : "light"}`}>
        <div className="burger__body">
          ...
          <Link onClick={handleBurgerClose} className={`burger__link ${isDarkTheme ? "" : "light"} mont-r`} to={"/doctors/"}>
            Лікарі
          </Link>
          <Link onClick={handleBurgerClose} className={`burger__link ${isDarkTheme ? "" : "light"} mont-r`} to={PRICES_ROUTE}>
           Ціни 
          </Link>
          <div onClick={() => handleSecondLevelMenuOpen("beforeAfter")} className={`burger__item-catalog ${isDarkTheme ? "" : "light"} mont-r`}>
            До/після <img src={isDarkTheme ? arrowGold : arrowBlack} alt="перейти до категорій в до/після" className="arrow__mobile" />
          </div>
          <Link onClick={handleBurgerClose} className={`burger__link ${isDarkTheme ? "" : "light"} mont-r`} to={CONTACTS_ROUTE}>
            Контакти
          </Link>
          <Link onClick={handleBurgerClose} className={`burger__link ${isDarkTheme ? "" : "light"} mont-r`} to={BLOG_ROUTE}>
            Блог
          </Link>
          <a
  ...
  <div onClick={handleSecondLevelMenuClose} className={`burger__body-second-back ${isDarkTheme ? "" : "light"} mont-r`}>
    <span>{"< Назад"}</span> <span>{activeCategory === "beforeAfter" ? "До/після" : "Послуги"}</span>
  </div>
  ...
  {activeCategory === "beforeAfter"
    ? navArrBeforeAfter.map((subCategory, idx) => (
        ...
        >
          {subCategory.title} <img src={isDarkTheme ? arrowGold : arrowBlack} alt="перейти до подкатегории" className="arrow__mobile" />
        </Link>
      ))
